...
const aliases = Object.keys(paths)
    ...
    aliasPaths: paths[alias].map((p) => {
        const parts = p.split('*');
        return {
            prefix: path_1.resolve(basePath, parts[0]),
    ...
    for (let j = 0; j < alen; j += 1) {
        ...
        if (modulePath.startsWith(prefix)) {
            ...
            for (let i = 0; i < len; i += 1) {
                ...
                const moduleSrc = apath.suffix ? path_1.resolve(apath.prefix, modulePathRel, apath.suffix) : path_1.resolve(apath.prefix, modulePathRel);
